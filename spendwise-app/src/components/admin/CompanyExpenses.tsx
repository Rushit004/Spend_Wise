...
const CompanyExpenseListItem: React.FC<{ expense: Expense; baseCurrency: string; isExpanded: boolean; onToggle: () => void; }> = ({ expense, baseCurrency, isExpanded, onToggle }) => {
...
const CompanyExpenses: React.FC = () => {
    const { user } = useAuth();
    const { getExpensesForCompany, getCompanyById, getAllUsersInCompany, getUserById } = useData();
    ...
    const expenses = getExpensesForCompany(user.companyId);
    ...
    return (
        <div>
            ...
            <p className="mt-1 text-gray-600 dark:text-gray-400">View all expenses submitted across the company.</p>
